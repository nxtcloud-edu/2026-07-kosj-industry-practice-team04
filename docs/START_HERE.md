...
- ✅ 역할 분담 확정 — FE-A 심송언 · FE-B 김재용 · BE 가동진 · AI/Admin 김성현
...
| 영역 라벨 | 이슈 수 | 내용 | 담당 |
|---|---|---|---|
| `frontend` | 11 | 시민 웹앱 화면 (신고 3단계·접근성·조회) | 심송언 (FE-A) · 김재용 (FE-B) |
| `backend` | 7 | 신고 API·통합/우선순위 로직·상태 모델 | 가동진 (BE) |
| `ai` | 2 | 이미지 분류 연동·정확도 점검 | 김성현 (AI/Admin) |
| `admin` | 3 | 관리자 검수 화면·대시보드 | 김성현 (AI/Admin) |
| `infra` | 2 | 배포 파이프라인·안정화 | 미정 (전원 협의) |

각 영역 이슈의 Assignee를 담당자로 지정하세요. 전원이 코드 리뷰에 참여합니다.
