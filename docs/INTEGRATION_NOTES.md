...
- **[API_CONTRACT.md](API_CONTRACT.md)가 프론트·백엔드 공통 기준**입니다. 경로·요청/응답 형태·상태 라벨·보안 규칙이 모두 여기 정리돼 있습니다.
- 관리자 API 6개 경로는 관리자 콘솔이 이미 호출 중이라 **변경 불가**, 시민 API는 계약대로 맞춰주세요.
- 계약을 바꿔야 하면 코드보다 **문서 PR을 먼저** 올려 합의합니다.
