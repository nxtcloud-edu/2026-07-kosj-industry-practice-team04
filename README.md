...
| `MOA_ADMIN_TOKEN` | (자동 생성 → 콘솔 출력) | `/api/admin/*` Bearer 인증 — 항상 잠김, **배포 시 고정값 필수** |
| `MOA_REPORT_LIMIT` | 5 | 같은 IP 시간당 신고 한도 (초과 429) |
...
- **지도 위치 보정 + 실주소** — Leaflet 지도에서 핀을 드래그하면 실제 주소(역지오코딩)가 따라 갱신 (SIR-002)
...
> 관리자 콘솔은 **항상 잠겨 있습니다.** `MOA_ADMIN_TOKEN`을 설정하지 않으면 백엔드가 시작할 때 임시 토큰을 만들어 **콘솔에 출력**하니, 그 값을 입력해 들어가면 됩니다 (재시작하면 바뀜). 배포 환경에서는 반드시 고정 토큰을 설정하세요.
